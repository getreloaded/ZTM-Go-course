...
func Average[T constraints.Float | constraints.Integer](arr []T) T {
	length := len(arr)
	var sum T
	for _, v := range arr {
		sum += v
...
func main() {
	...
	fmt.Println("Avg BWusage:", Average(dash.BandwidthUsage.amount))
	fmt.Println("Avg CPUtemp:", Average(dash.temp))
	fmt.Println("Avg MemUsage:", Average(dash.MemoryUsage.amount))
